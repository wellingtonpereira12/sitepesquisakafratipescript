...
export default function Dashboard() {
  ...
  const carregaML = async (pacote) => {
    try {
      ...
      const response = await fetch(process.env.NEXT_PUBLIC_LINKAPI+`/mercadoPagoCriaPagamento?text=${encodeURIComponent(pacote)}`, {
        method: 'POST',
        headers: {
  ...
  const handleDelete = async (text) => {
    try {
      console.log(text);
      const { ['kafra.token']: token } = parseCookies();
      await fetch(process.env.NEXT_PUBLIC_LINKAPI+`/deleteProcura?text=${encodeURIComponent(text)}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
  ...
  const handleAdd = async (event) => {
    ...
    try {
        const { ['kafra.token']: token } = parseCookies();
        const response = await fetch(process.env.NEXT_PUBLIC_LINKAPI+'/gravaProcura', {
            method: 'POST',
            headers: {
